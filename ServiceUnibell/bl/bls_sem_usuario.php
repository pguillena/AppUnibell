...
class bls_sem_usuario {
    ...
    public function SelectAll($p1,$p2,$p3){          
        ...
        try{
            ...
            $stmt = oci_parse($cnn,"begin PKG_MS_COBRANZA.LISTAR_S_SEM_USUARIO(:p1,:p2,:p3,:data); end;");
        ...
        try{
            $cnn = getConnectionOracle();
            $stmt = oci_parse($cnn,"begin PKG_MS_COBRANZA.RECUPERAR_USUARIO_MD5(:p1,:p2,:data); end;");
            $curs = oci_new_cursor($cnn);
            //Send parameters variable  value  lenght
            oci_bind_by_name($stmt, ':p1', $p1);
            oci_bind_by_name($stmt, ':p2', $p2);
