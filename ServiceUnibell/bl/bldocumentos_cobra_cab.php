...
class bldocumentos_cobra_cab {
    ...
    public function SelectAll($p1,$p2,$p3){          
        ...
        try{
            ...
            $stmt = oci_parse($cnn,"begin PKG_MS_COBRANZA.LISTAR_DOCUMENTOS_COBRA_CAB(:p1,:p2,:p3,:data); end;");
 ...
 public function CobranzaFlujo1($p1,$p2,$p3,$p4,$p5,$p6,$p7,$p8,$p9,$p10,$p11,$p12,$p13,$p14,$p15,$p16,$p17){          
      ...
      public function InsertCobranza(){
        ...
        try{            
            ...
            echo '{"status": 1, "message":"' .  $cont . ' Registro Insertado", "result":' . json_encode($pila) . '}';     
        }catch(Exception $e){
 ...
 public function ConciliarDepositos($p1,$p2,$p3,$p4,$p5,$p6,$p7){          
        ...
        try{
            $cnn = getConnectionOracle();
            $stmt = oci_parse($cnn,"begin PKG_MS_COBRANZA.LISTAR_DOCUMENTOS_COBRA_CAB(:p1,:p2,:p3,:p4,:p5,:p6,:p7,:data); end;");
            $curs = oci_new_cursor($cnn);
            //Send parameters variable  value  lenght            
            oci_bind_by_name($stmt, ':p1', $p1); 
            oci_bind_by_name($stmt, ':p2', $p2); 
