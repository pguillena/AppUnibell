...
class bls_gem_cliente {
    ...
    public function SelectAll($p1,$p2,$p3){          
        ...
        try{
            ...
            $stmt = oci_parse($cnn,"begin PKG_MS_COBRANZA.LISTAR_S_GEM_CLIENTE(:p1,:p2,:p3,:data); end;");
        ...
        public function SelectAllxCod($p1,$p2,$p3){
            ...
            try{
                $cnn = getConnectionOracle();
                $stmt = oci_parse($cnn,"begin PKG_MS_COBRANZA.LISTAR_S_GEM_CLIENTE_X_COD(:p1,:p2,:p3,:data); end;");
                $curs = oci_new_cursor($cnn);
                //Send parameters variable  value  lenght
                oci_bind_by_name($stmt, ':p1', $p1);
                oci_bind_by_name($stmt, ':p2', $p2);
