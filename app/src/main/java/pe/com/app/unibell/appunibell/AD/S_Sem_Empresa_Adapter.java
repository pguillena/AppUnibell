...
public class S_Sem_Empresa_Adapter extends ArrayAdapter<S_Sem_EmpresaBE> implements Filterable {
    public List<S_Sem_EmpresaBE> lstFiltrado;
    public List<S_Sem_EmpresaBE> lst;
    private AdapterFilter adapterFilter;

    public S_Sem_Empresa_Adapter(Context context, int resource, List<S_Sem_EmpresaBE> objects) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        final S_Sem_EmpresaBE s_sem_empresaBE = getItem(position);
        mainHolder.dfa_item1.setText(s_sem_empresaBE.getID_EMPRESA().toString());
        mainHolder.dfa_item2.setText(s_sem_empresaBE.getNOMBRE());
        return convertView;
    }
    ...
    @Override
    public S_Sem_EmpresaBE getItem(int position) {
    ...
    private class AdapterFilter extends Filter {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            if (constraint == null || constraint.length() <= 0) {
                ...
                List<S_Sem_EmpresaBE> localList = new ArrayList<>();
                String[] SSA=constraint.toString().toUpperCase().split(" ");
                for (S_Sem_EmpresaBE s_sem_empresaBE : lst) {
                    String todo = (s_sem_empresaBE.getNOMBRE().toUpperCase());
                    Boolean bfind = true;
                    for(String s : SSA){
                        if(!todo.contains(s)){
                            bfind = false;
