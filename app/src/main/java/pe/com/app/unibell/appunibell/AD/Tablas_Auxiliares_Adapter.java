...
public class Tablas_Auxiliares_Adapter extends ArrayAdapter<Tablas_AuxiliaresBE> implements Filterable {
    public List<Tablas_AuxiliaresBE> lstFiltrado;
    public List<Tablas_AuxiliaresBE> lst;
    private AdapterFilter adapterFilter;

    public Tablas_Auxiliares_Adapter(Context context, int resource, List<Tablas_AuxiliaresBE> objects) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null || !(convertView.getTag() instanceof Clientes_Adapter.MainHolder)) {
        ...
        final Tablas_AuxiliaresBE tablas_auxiliaresBE = getItem(position);
        mainHolder.dfa_item1.setText(tablas_auxiliaresBE.getCODIGO().toString().toUpperCase());
        mainHolder.dfa_item2.setText(tablas_auxiliaresBE.getDESCRIPCION().toUpperCase());
        return convertView;
    }
    ...
    @Override
    public Tablas_AuxiliaresBE getItem(int position) {
    ...
    private class AdapterFilter extends Filter {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            if (constraint == null || constraint.length() <= 0) {
                ...
                List<Tablas_AuxiliaresBE> localList = new ArrayList<>();
                String[] SSA=constraint.toString().toUpperCase().split(" ");
                for (Tablas_AuxiliaresBE tablas_auxiliaresBE : lst) {
                    String todo = (tablas_auxiliaresBE.getDESCRIPCION().toUpperCase());
                    Boolean bfind = true;
                    for(String s : SSA){
                        if(!todo.contains(s)){
                            bfind = false;
                    ...
                    if (bfind != false){
                        localList.add(tablas_auxiliaresBE);
                    }
                }
