...
public class CtaBnco_Adapter extends ArrayAdapter<CtaBncoBE> implements Filterable {
    public List<CtaBncoBE> lstFiltrado;
    public List<CtaBncoBE> lst;
    private AdapterFilter adapterFilter;

    public CtaBnco_Adapter(Context context, int resource, List<CtaBncoBE> objects) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        final CtaBncoBE ctaBncoBE = getItem(position);
        mainHolder.dfa_item1.setText(ctaBncoBE.getCODIGO().toString());
        mainHolder.dfa_item2.setText(ctaBncoBE.getDESCRIPCION());
        return convertView;
    }
    ...
    @Override
    public CtaBncoBE getItem(int position) {
    ...
    private class AdapterFilter extends Filter {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            if (constraint == null || constraint.length() <= 0) {
                ...
                List<CtaBncoBE> localList = new ArrayList<>();
                String[] SSA=constraint.toString().toUpperCase().split(" ");
                for (CtaBncoBE ctaBncoBE : lst) {
                    String todo = (ctaBncoBE.getDESCRIPCION().toUpperCase());
                    Boolean bfind = true;
                    for(String s : SSA){
                        if(!todo.contains(s)){
                            bfind = false;
